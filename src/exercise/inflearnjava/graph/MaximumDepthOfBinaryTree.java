...
public class MaximumDepthOfBinaryTree {
    ...
    private static int solve_recursive(TreeNode node) {
        if (node == null)
            return 0;

        int leftDepth = solve_recursive(node.left);
        int rightDepth = solve_recursive(node.right);
    ...
    private static int solve_bfs(TreeNode root) {
        if (root == null)
            return 0;
